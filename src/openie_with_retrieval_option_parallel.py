...
def openie(passage: str):
    messages = openie_prompt.format_prompt(passage=passage)
    not_done = True
    ...
    while not_done:
        try:
            ...
            if 'triples' not in response_content:
                triples = []
            else:
                triples = response_content['triples']
               
            if  not isinstance(named_entities, list) or not isinstance(triples, list):
                print("Response error: named_entities or triples not list")
                continue
            # if 'ner_description' not in response_content:
            #     ner_description = {}
            # else:
            #     ner_description = response_content['ner_description']
                
            
            openie_dict['named_entities'] = named_entities
            openie_dict['triples'] = triples
            # openie_dict['ner_description'] = ner_description
            ...
            openie_dict['named_entities'] = []
            openie_dict['triples'] = []
            if "Could not parse response content as the request was rejected by the content filter" in str(e): 
                not_done = False
...
if __name__ == '__main__':
    ...
    def extract_openie_from_passages(triple_json):
        ...
        for i, r in tqdm(triple_json, total=len(triple_json)):
            ...
            if i < len(existing_json):
                ...
                if auxiliary_file_exists:
                    ...
                    openie_dict, total_ner_tokens = openie(passage)
                    
                    # doc_entities, total_ner_tokens = named_entity_recognition(passage)
                    doc_entities = openie_dict['named_entities']
                    triples = openie_dict['triples']
                    # ner_description = openie_dict['ner_description']
                
                    try:
                        doc_entities = list(np.unique(doc_entities))
                        chatgpt_total_tokens += total_ner_tokens
                    except Exception as e:
                        print("doc_entities error: ", e)
                        doc_entities = []
                ...
                r['extracted_triples'] = triples
                # r['ner_description'] = ner_description
                new_json.append(r)
