...
from src.baselines.ircot_246 import parse_prompt
...
cot_system_instruction = ('You are an assistant who is good at reasoning and question answering. You need to analyze the given information according to the questions. '
                          'Response start after "Thought: ", where You will go through the reasoning process step by step to explain how you came to the conclusion. '
                          'Conclude with "Answer: " to present a concise, definitive response, devoid of additional elaborations.')


cot_system_instruction_no_doc = ('You are an assistant who is good at reasoning and question answering. You need to analyze the questions and then answer them. '
                          'Response start after "Thought: ", where You will go through the reasoning process step by step to explain how you came to the conclusion. '
                          'Conclude with "Answer: " to present a concise, definitive response, devoid of additional elaborations.')
...
def qa_read(query: str, passages: list, few_shot: list, client):
    ...
    if isinstance(few_shot, list):
        ...
        path_example = few_shot + '\n# Test begin:\n'

    try:
        user_prompt = ''
        if path_example != '':
            user_prompt += path_example
        user_prompt += 'Infomation:\n'
...
def parallel_qa_read(data: list, demos: list, args, client, output_path: str, total_metrics: dict, sample_id_set: set):
    ...
    def process_sample(sample):
        ...
        response, tokens = qa_read(query, retrieved, demos, client)
        total_tokens += tokens
        try:
            ...
            if result is not None:
                ...
                if sample_idx % 50 == 0:
                    with open(output_path, 'w') as f:
                        json.dump(data, f)
    print("Average number of documents used: ", total_use_doc_num/1000)
    print("token used: ", total_tokens)
    print("Average token used: ", total_tokens/1000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thread', type=int, default=10, help='the number of workers for parallel processing')
    parser.add_argument('--prefix', type=str, default='neuropath', help="retrieval method")
    args = parser.parse_args()

    output_path = f'exp/qa_{args.prefix}_{args.dataset}_{args.retriever}_{args.llm_model}_demo_{args.num_demo}_doc_{args.num_doc}.json'
