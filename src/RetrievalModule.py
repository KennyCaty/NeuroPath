...
class RetrievalModule:
    ...
    def __init__(self,
        ...
        try:
            if 'ckpt' in retriever_name:
                ...
                self.plm = AutoModel.from_pretrained(retriever_name, local_files_only=True)
    ...
    def encode_strings(self, strs_to_encode, pool_method):
        self.plm.to('cuda')
        tokenizer = AutoTokenizer.from_pretrained(self.retriever_name, local_files_only=True)
        ...
        with torch.no_grad():
            ...
            for i, string in tqdm(enumerate(strs_to_encode), total=len(strs_to_encode)):
                ...
                if max_pad_size * len(text_batch) > 1000 or num_strings_proc == len(strs_to_encode):
    ...
    # def retrieve_knn(self, queries, knowledge_base, k=1024):
