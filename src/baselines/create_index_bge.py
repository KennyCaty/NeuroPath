...
if __name__ == '__main__':
    ...
    if norm:
        vector_path = f'data/{args.dataset}/{args.dataset}_{model_label}_vectors_norm.npy'
        index_path = f'data/{args.dataset}/{args.dataset}_{model_label}_ip_norm.index'
    else:
        vector_path = f'data/{args.dataset}/{args.dataset}_{model_label}_vectors.npy'
        index_path = f'data/{args.dataset}/{args.dataset}_{model_label}_ip.index'
    ...
    if os.path.isfile(vector_path):
        ...
        for start_idx in tqdm(range(0, len(corpus_contents), batch_size), desc='encoding corpus'):
            ...
            batch_embeddings = model.encode(batch_passages, normalize_embeddings=True, device='cuda', batch_size=batch_size)
