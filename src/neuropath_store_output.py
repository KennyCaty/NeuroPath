...
from src.query_ner_vtp_parallel import named_entity_recognition, ner_vtp_extraction
...
class NeuroPath:
    ...
    def __init__(self, corpus_name='hotpotqa', extraction_model='openai', extraction_model_name='gpt-3.5-turbo-1106',
        ...
        # 2wikimultihopqa_facts_and_sim_graph_clean_facts_chatgpt_openIE.ents_only_lower_preprocess_ner_gpt-4o-mini.v3.subset
        # 2wikimultihopqa_facts_and_sim_graph_clean_facts_chatgpt_openIE.ents_only_lower_preprocess_ner.v3.subset.json
    ...
    ### 新增 =====
    def get_to_doc_path_dict(self, expand_ids_dict):
        to_doc_path_dict = {}
        """to_docs_path_dict内容示例：
        {
            # 到doc1的路径
            doc1: [
                [seed1, phrase1, doc1],
        ...
        """
        for key, value in expand_ids_dict.items():
            seed_doc_id = key
            p_idx_to_docs_dict = value
            for phrase_id, link_docs in p_idx_to_docs_dict.items():
                for doc_id in link_docs:
                    path = [seed_doc_id, phrase_id, doc_id]
                    to_doc_path_list = to_doc_path_dict.get(doc_id, [])
                    to_doc_path_list.append(path)
                    to_doc_path_dict[doc_id] = to_doc_path_list
        return to_doc_path_dict
                    
           
           
    def cal_doc_scores(self, query_embedding, to_doc_path_dict):
        """计算query与每到一个文档的路径的最大相似性分数"""
        doc_scores = {}
        all_pair_semantics = []  # 用于存储所有需要嵌入的语义对
        pair_to_doc_path = {}    # 用于记录每个语义对对应的文档和路径

        # 第一步：收集所有需要嵌入的语义对
        for doc_id, paths in to_doc_path_dict.items():
            ...
            for path in paths:
                ...
                seed_facts_ids_list = self.docs_to_facts_mat[[seed_id]].toarray()[0]
                non_zero_seed_facts_ids = np.nonzero(seed_facts_ids_list)[0].tolist()

                # 收集源节点的语义
                for facts_id in non_zero_seed_facts_ids:
                    if self.facts_to_phrases_mat[facts_id, phrase_id] != 0:
                        triple_json = self.fact_json[facts_id]
                        triple_semantics = triple_json['head'] + ' ' + triple_json['relation'] + ' ' + triple_json['tail']
                        source_semantics.append(triple_semantics)
                ...
                for facts_id in non_zero_target_facts_ids:
                    if self.facts_to_phrases_mat[facts_id, phrase_id] != 0:
                        triple_json = self.fact_json[facts_id]
                        triple_semantics = triple_json['head'] + ' ' + triple_json['relation'] + ' ' + triple_json['tail']
                        target_semantics.append(triple_semantics)

                # 生成语义对并记录其对应的文档和路径
                for source in source_semantics:
                    for target in target_semantics:
                        pair_semantic = f"{source} {target}"
                        all_pair_semantics.append(pair_semantic)
                        pair_to_doc_path[len(all_pair_semantics) - 1] = (doc_id, path)

        # 第二步：批量嵌入所有语义对
        if all_pair_semantics:  # 确保有需要嵌入的内容
            pair_semantics_embs = self.embed_model.encode(
                all_pair_semantics, normalize_embeddings=True, device='cuda', show_progress_bar=False, batch_size=256
            )

            # 第三步：计算每个语义对的相似度分数，并更新文档分数
            query_embedding = query_embedding.reshape(-1, 1)  # 确保 query_embedding 是列向量
            similarity_scores = np.dot(pair_semantics_embs, query_embedding).flatten()

            for idx, score in enumerate(similarity_scores):
                doc_id, _ = pair_to_doc_path[idx]
                old_score = doc_scores.get(doc_id, 0)
                if score > old_score:
                    doc_scores[doc_id] = score

        # 按分数升序排序
        sorted_docs = sorted(doc_scores.items(), key=lambda item: item[1], reverse=True)
        return sorted_docs  # 返回的是一个排序后的列表，每一个元素是一个元组(doc_id, score)
           
    def llm_gen_vtp(self, query, start_nodes):
        prompt = f"""Your task is to generate a concise graph pattern based on the given question and extracted named entities to guide KG path retrieval.
# Example 1:
Question : Who wrote the book that was published earlier, "The Great Gatsby" or "To Kill a Mockingbird"?
Entities : ["The Great Gatsby", "To Kill a Mockingbird"]
Output :
The Great Gatsby->written_by->Author->published_on->PublicationDate
To Kill a Mockingbird->written_by->Author->published_on->PublicationDate
# Example 2:
Question : Are the cities of Paris and Berlin located in the same country?
Entities : ["Paris", "Berlin"]
Output :
Paris->located_in->Country
Berlin->located_in->Country

Input :
Question : {query}
Entities : {start_nodes}
Output :
"""     
        try :
            chat_completion = self.client.invoke(prompt,temperature=0.0, )
            response = chat_completion.content
            total_tokens = chat_completion.response_metadata['token_usage']['total_tokens']
            # print("goal: ", response)
            
        except Exception as e:
            print(e, "llm gen pattern response error")
            response = ''
            total_tokens = 0
        return response, total_tokens   
        
        # You need to generate a one-sentence brief thought for the path retrieved in the current step, and ultimately provide the required JSON object.
...
Paths:
0: El Extrano Viaje->released in->1964; <expandable>: [1964]
...
3: Love in Pawn->released in->1953; <expandable>: [1953]
4: Love In Pawn->directed by->Charles Saunders; <expandable>: [Charles Saunders]
Output:
{
    "current_chain": "The director of El Extrano Viaje is Fernando Fernan Gomez. And the director of Love In Pawn is Charles Saunders",
    "valid_ids": [1,4],
...
Paths:\n"""
        ...
        return  response, total_tokens, llm_time, train_json
    ...
    def pruning_bk(self, filter_q, path_dict, filter_k=30):
        print("filter_q: ", filter_q)
        # start_time = time.time() 
        if len(path_dict['paths']) <= filter_k:
            return path_dict
        ...
        path_dict['paths'] = [path_dict['paths'][i] for i in topk_indices]
        path_dict['link_phrases'] = [path_dict['link_phrases'][i] for i in topk_indices]
        path_dict['visited_docs'] = [path_dict['visited_docs'][i] for i in topk_indices]
    ...
    def BFS_by_llm(self, path_dict, query_embedding, query, vtp=None): 
        total_tokens = 0
        # 第一轮
        if path_dict['paths'] == []:
            new_link_phrases = []
            # print("第一轮初始节点： ", path_dict['link_phrases'])
            # for idx in path_dict['link_phrases']:
            #     print(self.phrases[idx])
            expand_phrase_ids = self.expand_seed(path_dict['link_phrases'])
            # print("第一轮扩展后节点 ", expand_phrase_ids)
            # for idx in expand_phrase_ids:
            #     print(self.phrases[idx])
            # for link_phrase_id in path_dict['link_phrases']:
            ...
            # 相似度剪枝
            # path_dict = self.pruning(query_embedding, path_dict)
            # vtp = query + '\n' + vtp
            path_dict = self.pruning(0, vtp, path_dict)
            # path_dict = self.pruning(query, path_dict)
            ...
            response_json, tokens, llm_time, train_json = self.llm_path_track(path_prompt, query, vtp)
            total_tokens += tokens       
            ...
            try:
                ...
                path_dict['thought'] = response_json['thought']
                ...
                path_dict['thought'] = ''
                flag = 0
            return path_dict, flag, total_tokens, llm_time, train_json
        ...
        # 之后的轮数
            ...
            for i in range(len(path_dict['paths'])):  # 对于每一个可扩展路径而言，分别扩充路径
                ...
                if path_dict['paths'][i] not in locked_path:
                    path = path_dict['paths'][i]
                    # print("正在扩展: ", path)
                    # link_phrase_id = path_dict['link_phrases'][i]
                    seed_phrase_id = path_dict['link_phrases'][i]
                    # 扩展相似短语 （共指消解）
                    # print("第二 轮初始节点： ", seed_phrase_id)
                    expand_phrase_ids = self.expand_seed([seed_phrase_id])
                    # print("第二轮扩展后节点 ", expand_phrase_ids)
                    # print("此路径连接的短语: ", expand_phrase_ids)
    ...
    def path_filter(self, path_dict, query_embedding):
        paths = path_dict['paths']
        path_embeddings = self.embed_model.encode(paths, normalize_embeddings=True, device='cuda', show_progress_bar=False, batch_size=256
            )
        # 
        ...
        similarity_scores = np.dot(path_embeddings, query_embedding).flatten()
        top_k = 3
        topk_indices = np.argsort(similarity_scores)[-top_k:][::-1]  
        # print("Top-k 最大值索引:", topk_indices)
        return topk_indices
    ...
    def get_path_filtered_docs(self, query_embedding, seed_phrase_ids, query, vtp=None):
        ...
        {
            seed_doc_id1: {
                phrase_id1: [doc1, doc2...],
                phrase_id2: [doc5, doc7...],
            },
            
        }
        """
        unique_phrase_ids_set = set(seed_phrase_ids)
        # expand_ids_dict = {}  # doc_idx: set()
        p_idx_to_docs_dict = {} # p_idx: list() 有这个phrase id 到 其他文档id的dict，用于存储路径, 每一个key是seed文档的一个实体，v是有这个实体可以连接的文档id列表
        # 初始化
        path_dict = {}
        path_dict['paths'] = []
        ...
        count = 0
        total_tokens = 0
        # vtp, tokens = self.llm_gen_vtp(query, query_ner_list)
        # vtp = vtp.replace("->", " ")
        # vtp = vtp.replace("_", " ")
        cur_vtp = query
        # cur_vtp = vtp
        ...
        while count < max_layer and flag==1:
            count += 1
            path_dict, flag, tokens, llm_time, train_json = self.BFS_by_llm(path_dict, query_embedding, query, cur_vtp)
            if train_json != {}:
                train_list.append(train_json)
            llm_time_for_one_q += llm_time
            total_tokens += tokens
            # all_thought = path_dict['thought'] + path_dict['current_chain']
            all_thought = path_dict['current_chain'] + path_dict['expansion_requirements']
            # all_thought = path_dict['current_chain']
            # all_thought = path_dict['expansion_requirements']
            # cur_vtp = vtp + '\n' + path_dict['current_chain']
            # cur_vtp = vtp + path_dict['current_chain']
            cur_vtp = path_dict['expansion_requirements']
            # cur_vtp = query
            # print("当前想法： ", path_dict['current_chain'])
            if flag==0 and path_dict['valid_paths'] != []:
                print("大模型决定了路径")
            elif path_dict['valid_paths'] == [] and path_dict['need_expand'] == []:
                print("大模型没有找到有效路径")
                flag = 0
            # path_dict, flag = self.BFS_by_emb(path_dict, query_embedding)
            
        # path_dict, flag, tokens = self.lazy_track_by_llm(path_dict, query_embedding, query, vtp)
            
            
        # print(query)
        # print("最终path：", path_dict['paths'])
        # assert False
        
        # # 校验步骤
        # if path_dict['paths'] != []:
        #     path_dict, check_tokens = self.final_check(path_dict)
        ...
        # assert False
        return candidate_docs, candidate_paths, total_tokens, all_thought, llm_time_for_one_q, train_list
       
    
    def final_check(self, path_dict):
        # print("开始校验")
        paths = path_dict['paths']
        docs = path_dict['visited_docs']
        if paths == []:
            return path_dict, 0
        path_prompt = ""
        for i in range(len(paths)):
            path_prompt += f'{i}: ' + paths[i] + '\n'
        prompt = f"""Your task is to filter the valid clues that provide direct or indirect help in answering the query from the following clues. filter out redundant and invalid clue ids. Sort the filtered IDs by their helpfulness to the query from most to least. Return in JSON format.
Cluse:
{path_prompt}
Output JSON Format:
{{
    "valid_ids": [IDs of valid clues]
}}
Output:
        """
        # print(prompt)
        ...
        try :
            ...
            print(e, "llm final check response error")
            if response=='':
                print("json.loads error, llms response is not a json")
            else:
                print(response)
            ...
            total_tokens = 0
        path_dict['paths'] = [paths[i] for i in response["valid_ids"]]
        path_dict['visited_docs'] = [docs[i] for i in response["valid_ids"]]
        return path_dict, total_tokens
    ...
    def rank_docs(self, query: str, top_k, seed=1, expand_num=1):  # 
        ...
        if self.graph_alg=='kg_path':
            ...
            if len(query_ner_list) > 0: # 使用Path
                ...
                # 拿到新排名的doc id
                candidate_doc_ids, candidate_paths, total_tokens, all_thought, llm_time_for_one_q, train_list = self.get_path_filtered_docs(None, linked_phrase_ids, query, None)
                # sorted_other_doc_ids = sorted_other_doc_ids[: expand_num]
                # print(sorted_other_doc_ids)
                ...
                # query_embedding = self.embed_model.encode([all_thought], normalize_embeddings=True, device='cuda', show_progress_bar=False)
                # assert False
                ...
                llm_time_for_one_q = 0
                train_list = []
        return doc_ids[:top_k], doc_scores[:top_k], candidate_doc_ids, candidate_paths, total_tokens, llm_time_for_one_q, train_list
    
    
    def gen_iter_query(self, query: str):
        prompt = """
Your task is to generate the collection steps of the answer based on the given question and the important entities of the steps to guide the retrieval. Each step is as simple as possible. Return JSON format.
For simple queries, just generated one step. For complex queries that require multiple hops, the answer to the previous step is marked as #id.
# Example:
Question : When was Lady Godiva's birthplace abolished?
Output :
{{
    "#1": {{
            "step": "Where was Lady Godiva's birthplace?",
            "entities": ["Lady Godiva"]
        }},
    "#2": {{
            "step": "When was #1 abolished?",
            "entities": ["#1"]
        }}
}}
...
"""
        ...
        try :
            ...
            total_tokens = chat_completion.response_metadata['token_usage']['total_tokens']
            response = extract_json_dict(response)
            if response.get('#1', {}) == {} or response['#1'].get('step', '') == '' or response['#1'].get('entities', []) == []:
                print('gen iter query key error')
                response = None
        except Exception as e:
            print('gen iter query error: ', e)
            response = None
        return response, total_tokens
        ...
        if self.dpr_only:
            ...
            try:
                if query in self.named_entity_cache:
                    ...
                    # virtual_target_path = self.named_entity_cache[query]['pattern']
                    ...
                    # virtual_target_path = eval(query_ner_json)['pattern']
                ...
                query_ner_list = []
                virtual_target_path = ''
        # return query_ner_list, virtual_target_path
    ...
    def get_dpr_doc_embedding(self):
        ...
        if os.path.exists(cache_filename):
            ...
            self.doc_embeddings = []
            self.doc_embedding_mat = self.embed_model.encode(self.dataset_df['paragraph'].tolist(), normalize_embeddings=True, device='cuda', batch_size=64)
